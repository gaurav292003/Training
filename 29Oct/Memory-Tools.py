...
while True:
    ...
    if "weather" in user_input.lower():
        # Extract city from the user's input (assuming the user asks for the weather in a specific city)
        if "in" in user_input.lower():
            city = user_input.split("in")[-1].strip()  # Get city after "in"
            city = ''.join(e for e in city if e.isalnum())  # Remove any non-alphanumeric characters
        else:
            city = None

        if city:
            # Call weather function with the city name
            weather_info = weather(city)
            print("Agent:", weather_info)
            memory.save_context({"input": user_input}, {"output": weather_info})
        else:
            print("Agent: Please specify a city. Example: 'What's the weather in Dubai?'")
        continue

    # Default: use LLM (if no other command matches)
    try:
